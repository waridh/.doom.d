...
:END:
This section contains the configuration for ~doom emacs~ once the initial packages and modules have been loaded.
...
#+caption: Description of the different templates.
| Name       | Hot Key | Description                                       |
|------------+---------+---------------------------------------------------|
| Encryption | e       | Used to generate encrypted ~org-roam~ entries.      |
|            |         | This is required for notes that cannot be seen by |
|            |         | anyone else.                                      |
|------------+---------+---------------------------------------------------|
#+begin_src elisp
...
The following configuration allows for notes in ~org-roam~ to be used as a task reference page.
#+begin_src elisp
...
#+end_src
**** Hook registration
In this section, the relevant hooks for ~org-roam~ are registers so that it would load after the package has been loaded by ~doom emacs~.
#+begin_src elisp
(after! org-roam
